...
You can use JavaScript getters to create computed properties that feel like native properties on your store. This provides a more ergonomic API compared to calling methods:
...
const store = create(
  ...
  {
    get double() { // [!code hl]
      return store.get().count * 2; // [!code hl]
    }, // [!code hl]
    increment: () => store.set(state => ({ count: state.count + 1 }))
  }
);
...
console.log(store.double); // [!code hl]
//                ^^^^^^
// @log: 0
...
Getters are particularly useful for computed values that should always reflect the current state:
...
const store = create([] as Todo[], {
  ...
  get total() { // [!code hl]
    return store.get().length; // [!code hl]
  }, // [!code hl]
  get completed() { // [!code hl]
    return store.get().filter(t => t.completed).length; // [!code hl]
  }, // [!code hl]
  get active() { // [!code hl]
    return store.get().filter(t => !t.completed).length; // [!code hl]
  } // [!code hl]
...
console.log(store.total); // [!code hl]
// @log: 0

store.addTodo("Buy groceries");
console.log(store.active); // [!code hl]
// @log: 1
```

<Callout type="tip">
  Use getters for computed values that are derived from state, and regular
  methods for actions that perform side effects or state updates. This creates
  a clear distinction in your API.
</Callout>
