...
describe("options: version/migrate", () => {
  ...
  test("hydrated is true even if version mismatch and no migrate", async () => {
    const storage = createMockAsyncStorage();
    await storage.setItem("stav/async-persist", [{ count: 10 }, 1]);
...
describe("edge cases", () => {
  ...
  test("idempotent set calls don't trigger persist", async () => {
    const storage = createMockAsyncStorage();
    ...
    expect(await storage.getItem("stav/async-persist")).toEqual(null);
  });
});
